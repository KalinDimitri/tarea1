    ...
    productsHTML.addEventListener("click", function (e) {
        if (e.target.classList.contains("bx-plus")) {
            ...
            if (db.cart[productFind.id]) {
                ...
                if (productFind.quantity === 0) return alert("No tenemos mas en la bodega");
...
function handleProductsInCart(db) {
    ...
    cardProducts.addEventListener("click", function (e) {
        ...
        if (e.target.classList.contains("bx-trash")) {
            const id = Number(e.target.parentElement.id);
            const response = confirm("Desesas eliminar este producto?")
            if (!response) return;
            delete db.cart[id];
        };
        window.localStorage.setItem("cart", JSON.stringify(db.cart))
...
function handleTotal(db) {
    ...
    btnBuy.addEventListener("click", function () {
        if (!Object.values(db.cart).length) return alert("selecciona al menos un producto para hacer tu compra.");
        const response = confirm("Seguro que quieres comprar? si es asi Gracias por tu compra")
        if (!response) return;
...
function filterProducts() {
    ...
    const btns = document.querySelectorAll(".button_filter");
}
function loader() {
